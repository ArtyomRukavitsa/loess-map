...
ORD_DEP = ["loess", "loess-like loam", "loess-like silty loam", "alluvial sands", "marine deposits",
           "slope deposits", "alluvium", "volcanic ash", "till"]
...
for r in list(ws.values)[1:]:
    ...
    if lat is None or lon is None:
        continue
    rid += 1
    thmin, thmax = fnum(r[C["thickness_min_m"]]), fnum(r[C["thickness_max_m"]])
    ...
    rec = {
        ...
        "typeExcavationRaw": "ND", "typeExcavationTags": ["ND"],
        "geomModern": "ND", "geomPosition": "ND",
...
for (lat, lon), recs in groups.items():
    ...
    m = {
        ...
        "typeExcavation": ["ND"], "depositType": dep or ["ND"],
...
SUMMARY = {"rawRecords": rid, "markers": len(DATA), "duplicateGroups": dup,
           "accuracyPresent": rid, "accuracyMin": 5000.0, "accuracyMax": 5000.0}
